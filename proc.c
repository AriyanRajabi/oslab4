...
void scheduler(void)
{
  ...
  for (;;)
  {
    ...
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
    {
      if (p->state != RUNNABLE || p->sem != 0) // continue if a process is waiting for a semaphore
        continue;
...
int sem_release(int i)
{
  ...
  if (semaphores[i].count > 0)
  {
    ...
    memmove(&semaphores[i].queue[0], &semaphores[i].queue[1], (semaphores[i].count - 1) * sizeof(struct proc *));
    semaphores[i].count--;
